...
export class TextureGenerator {
  static createDiceTexture(number: number): THREE.CanvasTexture {
  ...
  static createDiceMaterials(numFaces: number): THREE.MeshStandardMaterial[] {
    ...
    if (numFaces === 10) {
      ...
      for (let i = 1; i <= 10; i++) {
        const texture = this.createDiceTexture(i);
      ...
      for (let i = 0; i < numFaces; i++) {
        const texture = this.createDiceTexture(i + 1);
        materials.push(new THREE.MeshStandardMaterial({
          map: texture,
          roughness: TEXTURE.ROUGHNESS,
          metalness: TEXTURE.METALNESS,
